...
            msg.textContent = "Please enter both TRN and password.";
            return;
        }
...
       
        // 1. Find the user by TRN (which is the input username)
        const user = users.find(u => u.trn === username);
...
            localStorage.setItem("loginLockout", JSON.stringify(lockoutData));
            msg.className = "message error";
            msg.textContent = `Invalid TRN or password. ${attemptsLeft} attempt(s) remaining.`;
            return;
        }
